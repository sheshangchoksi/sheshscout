...
def score_long(snap, params, hourly_ctx=None, index_change_pct=None):
    try:
        ...
        dist_from_day_high = ((high_price - current_price) / high_price) * 100 if high_price else 0
        ...
        # Gap check: a gap-up open that's STILL glued to the day's high (no
        # pullback anywhere in the session) is the textbook "don't chase"
        # case -- a gap-up that has pulled back and is now resuming is the
        # textbook entry the pullback rule is looking for. Needs yesterday's
        # close, so only evaluated when we have real daily history for it.
        gap_pct = ((open_price - daily_close[-1]) / daily_close[-1]) * 100 if len(daily_close) and daily_close[-1] else 0.0
        is_gap_up = gap_pct > params["gap_threshold"]
        gap_chase = is_gap_up and dist_from_day_high < 0.3
        gap_with_pullback = is_gap_up and not gap_chase
        ...
        # Volume ratio, corrected for time of day: comparing partial-day
        # cumulative volume against a 5-day *full-day* average understates
        # the ratio all morning and inflates it all afternoon, so an early
        # genuine volume burst could get filtered out purely for being
        # early. Scale the average down by how much of the session has
        # elapsed so the ratio means the same thing at 10 AM as at 2 PM.
        ...
        # hourly trend direction -- "near day low" only means something if
        # today's low is also close to a level other people are watching,
        # and a 5-min move that fights the hourly trend is a classic
        # reversal setup rather than a confirmed one.
        dist_from_support = None
        hourly_trend_pct = None
        if hourly_ctx is not None and hourly_ctx.get("support"):
        ...
        if gap_with_pullback:
            conditions_met.append("Gap-up with pullback confirmed")
        elif gap_chase:
            warnings.append("gap-up, no pullback (chase risk)")
...
def render() -> None:
    ...
    with st.sidebar.expander("Advanced Thresholds"):
        ...
        dist_from_support_threshold = st.slider(
            "Dist from Real Support (%)", 0.5, 5.0, 2.0, 0.5, key=sskey(MODE_KEY, "dist_support_th"),
            help="Support/resistance from 5 days of hourly bars — not today's own intraday low.",
        )
    ...
    def fetch_and_analyze(rec):
        ...
        # Hourly S/R + trend (per-symbol, cached 20 min) and the broad
        ...
        analysis = score_long(snap, params, hourly_ctx, index_change_pct)
        if analysis is None:
        ...
        # conditions, gating, or sorting; only shown in the table/detail
        # view for the person's own judgement. change_pct here is today's
        # single-session move, so the extreme/moderate bar is calibrated
        # for a same-day scale (an 8%+ single-session move is already a
        # lot, unlike a multi-day swing move).
